	...
	_ramNumber{ramNumber}
{
		...
		if ((iter->find(CAPACITY_IDENTIFIER_STRING) != std::string::npos) && (iter->find(CAPACITY_IDENTIFIER_STRING) == 0)) {
			...
			if (capacityString == "") {
				...
				try {
					...
					auto capacityMB = capacity / BYTES_PER_MEGABYTE;
					this->_capacityStr = toString(capacityMB) + "MB (" + toString(capacity) + " Bytes)";
					this->_capacity = static_cast<uint32_t>(capacityMB / 1000);
				} catch (std::exception &e) {
					(void)e;
					this->_capacityStr = capacityString + " Bytes";
				}
			}
		}
    ...
    if ((this->_capacityStr == "") || (this->_capacityStr == " Bytes")) {
		this->_capacityStr = "Unknown";
    }
...
std::string& RAMInfo::manufacturer() 
{
...
std::string& RAMInfo::capacityStr() 
{
	return this->_capacityStr;
}

std::string RAMInfo::capacityStr() const
{
	return this->_capacityStr;
}

std::string& RAMInfo::serialNumber() 
{
...
uint32_t RAMInfo::capacity() const
{
