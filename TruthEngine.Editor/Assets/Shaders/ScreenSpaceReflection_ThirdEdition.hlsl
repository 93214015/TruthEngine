...
float4 ps(VertexOut _PixelIn) : SV_Target
{
    ...
    float _PosHS0w = mul(_PosV, Projection).w;
    float _PosHS1w = mul(_EndV, Projection).w;
    
    float _K0 = 1 / _PosHS0w;
    float _K1 = 1 / _PosHS1w;
    
    float3 _Q0 = (_PosV * _K0).xyz;
    float3 _Q1 = (_EndV * _K1).xyz;
    ...
    _Delta = max(_Delta, 0.001f);
    float _InvDelta = 1.0f / _Delta;
    
    float2 _dPixel = float2(_DeltaX, _DeltaY) * _InvDelta;
    float3 _DeltaQ = _Q1 - _Q0;
    float3 _dQ = _DeltaQ * _InvDelta;
    float _DeltaK = _K1 - _K0;
    float _dK = _DeltaK * _InvDelta;
    ...
    for (i = 0; i < _Delta; ++i)
    {
        ...
        float _rQ = _Q.z + (_dQ.z * 0.5f);
        float _rK = _K + (_dK * 0.5f);
        
        _RayMinMaxDepth.y = _rQ / _rK;
    ...
    for (i = 0.0f; i < _Steps; ++i)
    {
        ...
        _Q = lerp(_Q0, _Q1, _Search1);
        _K = lerp(_K0, _K1, _Search1);
        
        float _SampleDepthP = tDepth.Sample(sampler_point_wrap, _UV.xy);
        float _SampleDepthV = ConvertToLinearDepth(_SampleDepthP, ProjectionValues.z, ProjectionValues.w);

        
        _RayMinMaxDepth.y = _Q.z / _K;
